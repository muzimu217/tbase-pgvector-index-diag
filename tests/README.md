...
## Zero-parameter audit smoke

The catalog audit was executed on the authorized OpenTenBase endpoint with the
same SQL sequence used by the test file:

```bash
PGHOST=127.0.0.1 PGPORT=22201 PGUSER=otbtest PGDATABASE=ivfflat_test \
  psql -X -q -v ON_ERROR_STOP=1 -f tests/sql/002_audit_zero_param.sql
```

The transaction created a 1000-row `vector(3)` table, one IVFFlat index and one
HNSW index, then called `audit_all_vector_indexes()` with zero arguments. The
raw output contains `rows=1000`, `dims=3`, `lists=10` for the IVFFlat row and
`index_type=hnsw` for the HNSW row; the transaction ends with `ROLLBACK`.

Raw output: `audit-zero-param-20260818.log` (SHA256:
`14da8373ae8f36b91f74ff52abf1bd900c451de1c17eb89d86e1e06699d105e7`).
